...
cd %MICO_ROOT_PATH%
b2.exe -j%JOBS% threading=multi pch=off address-model=32 link=shared runtime-link=shared variant=debug variant=release debug-symbols=on install
b2.exe -j%JOBS% threading=multi pch=off address-model=32 link=static runtime-link=static variant=debug variant=release debug-symbols=on install
