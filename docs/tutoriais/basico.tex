...
As seguintes linhas abaixo devem ser adicionadas neste arquivo:
...
registro e o serviço de sessão, são acessados diretamente através dos 
stubs disponibilizados. No caso do Mico a API para o serviço de registro
é descrita através do arquivo \lstinline!stubs/mico/registry_service.h!, 
e para o serviço de sessão é o arquivo \lstinline!stubs/mico/session_service.h!. 
Quanto ao Orbix, o serviço de registro é representado por 
\lstinline!stubs/orbix/registry_service.hh! e 
o serviço de sessão por \lstinline!stubs/orbix/session_service.hh!.
...
\subsubsection{Stub do serviço de registro}

\begin{lstlisting}
  RegistryIdentifier _cxx_register(const ServiceOffer& aServiceOffer)
\end{lstlisting}
...
identificação do mesmo é retornada através de uma string.
...
\begin{lstlisting}
  ServiceOfferList* find(const FacetList& facets);
\end{lstlisting}
...
A API utiliza o log criado no tecgraf, que se encontra atualmente no projeto 
 cpputils. O Log deve ser definido na chamada ao Openbus::init(). Abaixo temos a
 documentação encontrada na chamada init.
 
\begin{itemize}
 \item OpenbusDebug:
  \subitem ALL Ativa todos os níveis de verbose.
  \subitem ERROR Ativa o nível ERROR do debug.
  \subitem INFO Ativa o nível INFO do debug.
  \subitem WARNING Ativa o nível WARNING do debug.
 \item OpenbusDebugFile: Caminho completo ou relativo do arquivo que 
