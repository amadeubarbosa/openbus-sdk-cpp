...
\emph{\$OPENBUS\_HOME/libpath/\$TEC\_UNAME}.
  ...
  na pasta \emph{certificates} do barramento, e o certificado referente ao
  serviço de acesso deve   ser obtido junto ao administrador do barramento e
  colocado na pasta atual, em conjunto  da chave privada que foi gerada.
  Maiores informações sobre a geração de chaves e certificados digitais 
  podem ser encontradas no site do OpenBus. \url{https://jira.tecgraf.puc-rio.br/confluence/display/OpenBus/Geracao+de+Chaves+e+Certificados+Digitais}
...
\subsubsection{openbus.h}\label{api}
...
desejado. 
Um componente pode ser construido utilizando o construtor de
ComponentContext, e de posse desse componente ele pode
adicionar uma ou mais facetas, como mostra o código~\ref{lst:componentcontext}.

\begin{labeledcode}[c++]{componentcontext}{Construção de componente}
  /* Definicao do componenteId. */
  ...
  /* Adição de faceta IHello */
  componentContext->addFacet("IHello", "IDL:demoidl/hello/IHello:1.0", new HelloImpl);
\end{labeledcode}
...
direcionadas para o serviço ofertado, o que pode ser feito com o uso do método da API \emph{run()} descrito anteriormente na Seção \ref{api}.
...
// conectando via usuário e senha
...
o usuário possa obter um objeto CORBA que represente o serviço desejado. De
posse do objeto CORBA, o usuário deve efetuar um narrow para mapeá-lo para 
a interface desejada, e daí em diante o programador estará apto a utilizar o 
serviço. Vide o código~\ref{lst:getFacet}.

\begin{labeledcode}[c++]{getFacet}{Acesso ao serviço desejado.}
// obtendo o serviço IHello
...
// mapeando para a interface desejada
demoidl::hello::IHello_var hello = demoidl::hello::IHello::_narrow(obj);
\end{labeledcode}

Os códigos completos podem ser conferidos na demos dos projeto.
