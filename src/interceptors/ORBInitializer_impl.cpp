...
void ORBInitializer::pre_init(PortableInterceptor::ORBInitInfo *info) {
  ...
  _clientInterceptor = std::auto_ptr<ClientInterceptor> 
    (new ClientInterceptor(_slotId_connectionAddr,_slotId_joinedCallChain,_slotId_ignoreInterceptor, 
    _codec.in()));
  ...
  _serverInterceptor = std::auto_ptr<ServerInterceptor> (
    new ServerInterceptor(piCurrent.in(), _slotId_connectionAddr, _slotId_joinedCallChain,
