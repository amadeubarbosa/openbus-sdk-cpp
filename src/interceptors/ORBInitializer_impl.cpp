...
void ORBInitializer::pre_init(PortableInterceptor::ORBInitInfo *info) 
{
  ...
  _clientInterceptor.reset(new ClientInterceptor(
    _slotId_requesterConnection, _slotId_joinedCallChain, 
    _slotId_ignoreInterceptor, _codec));
  ...
  _serverInterceptor.reset(new ServerInterceptor(
    piCurrent, _slotId_requesterConnection, _slotId_receiveConnection, 
